...
public:
    ...
    virtual void parse() {
        ...
        if (mem_quota_flag) {
            ...
            mem_quota = (sys_mem) * mtcfg.socket_num / get_max_socket_num();
        }
...
class RWPTWalkOptionHelper
{
private:
    args::ValueFlag<int> epoch_num_flag;
    args::ValueFlag<uint64_t> walker_num_flag;
    args::ValueFlag<int> walk_len_flag;
    args::ValueFlag<double> rastart_probability_flag;
    args::ValueFlag<uint16_t> walker_size_flag;
    args::ValueFlag<vertex_id_t> degree_flag;
public:
    int epoch_num;
    uint64_t walker_num;
    int walk_len;
    double start_prb;
    uint16_t walker_size;
    vertex_id_t degree_bod;
    RWPTWalkOptionHelper(args::ArgumentParser &parser):
        ...
        rastart_probability_flag(parser, "startprb", "the rastart probability", {"sp"}),
        walker_size_flag(parser, "walkersize", "how many walkers each walekr array stores", {"ws"}),
        degree_flag(parser, "degree_boundary", "the boundary to partition high-low vertex", {"db"})
    {
    }
    virtual void parse() {
        ...
        CHECK(rastart_probability_flag);
        start_prb = args::get(rastart_probability_flag);
        walk_len = int(1/start_prb) + 1;
        if(walk_len_flag)
        {
        ...
        if(walker_size_flag)
        {
            walker_size = args::get(walker_size_flag);
        }
        if(degree_flag)
        {
            degree_bod = args::get(degree_flag);
        }
        LOG(WARNING) << block_mid_str() << "Restart prb: " << start_prb;
    }
...
class RWPTWalkOptionParser: public OptionParser, public NumaOptionHelper, public GraphOptionHelper, public RWPTWalkOptionHelper
{
public:
    RWPTWalkOptionParser():
        OptionParser(),
        NumaOptionHelper(parser),
        GraphOptionHelper(parser),
        RWPTWalkOptionHelper(parser)
    {}
    virtual void parse(int argc, char** argv) override {
       OptionParser::parse(argc, argv);
       NumaOptionHelper::parse();
       GraphOptionHelper::parse();
       RWPTWalkOptionHelper::parse();
    }
};
